...
export class Player {
  ...
  private videoProgressBar: HTMLDivElement;
  private soundBtn: HTMLButtonElement;
  private soundProgressBar: HTMLDivElement;
  private fullScreenBtn: HTMLButtonElement;
  private timeDisplay: HTMLDivElement;
  private url = { thumb: "", video: "" };
  private options = { duration: 0, currentTime: 0, volume: 0 };
  ...
  private onLoadedData() {
    this.options.duration = this.video.duration ?? 0;
  ...
  // 영상 진행바 클릭 콜백
  private onVideoProgressBarClick(e: MouseEvent) {
    const target = (e?.currentTarget ?? e?.target) as HTMLDivElement;
    let result = this.options.duration * (e?.offsetX / target?.clientWidth);
  ...
  // 볼륨바 클릭 콜백
  private onSoundProgressBarClick(e: MouseEvent) {
    const target = (e?.currentTarget ?? e?.target) as HTMLDivElement;
    let result = e?.offsetX / target?.clientWidth;
  ...
  private progressRender() {
    // 영상 퍼센트
    const videoPercent =
      (this.options.currentTime / this.options.duration) * 100;
